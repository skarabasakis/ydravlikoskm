...
// Cufon.replace('h1,h2,h3,h4,h5,h6,.heading',{ fontFamily: 'Quicksand' });
	...
	if (jQuery('.featured_item').length > 0 )jQuery('#featured').levitate({
			...
			opacity:0.8,					// opacity of second row
...
(function($)
{
	$.fn.levitate = function(default_options)
	{
		...
		return this.each(function()
		{
			...
			var $active = $(".featured_item_active");
			var $active_xmin = $active.offset().left;
			var $active_xmax = $active_xmin + $active.width();

			$items.click(function(e)
			{


				if (e.pageX < $active_xmin || e.pageX > $active_xmax) {
					e.preventDefault();
					if (! $animating)
					{
